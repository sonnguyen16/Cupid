...
namespace Cupid
{
    public partial class form_TaoMaGiamGia : Form
    {
        string tk;
        bool check_giaodien, check_ngonngu;
        public form_TaoMaGiamGia(string tk)
        {
            InitializeComponent();
            this.tk = tk;
        ...
        private void button_Tao_Click(object sender, EventArgs e)
        {
            if (KiemTra())
            {
                ...
                bool data = MaGiamGiaDAO.Instance.ThemMaGiamGia(textbox_GiamGia.Texts, float.Parse(textbox_GiamGia.Texts), dt1, dt2);
                if (data)
                {
                    this.Close();
                    var f = (form_KhuyenMai)this.Owner;
        ...
        private bool KiemTra()
        {
            if (!check_ngonngu)
            {
                if (!MaGiamGiaDAO.Instance.KiemTraMaKM(textbox_GiamGia.Texts))
                {
                    label_Loi.Text = "Mã giảm giá đã tồn tại";
                    return false;
                }
            ...
            else
            {
                if (!MaGiamGiaDAO.Instance.KiemTraMaKM(textbox_GiamGia.Texts))
                {
                    label_Loi.Text = "Discount code already exists";
                    return false;
                }
                if (textbox_GiamGia.Texts.Trim().Equals(""))
                {
                    label_Loi.Text = "You have not entered a discount code";
                    return false;
                }
                if (textbox_GiamGia.Texts.Trim().Equals(""))
                {
                    label_Loi.Text = "You have not entered the reduction %";
                    return false;
                }
                int a = DateTime.Compare(dateTimePicker_NgayKetThuc.Value, dateTimePicker_NgayBatDau.Value);
                if (a < 0)
        ...
        private void form_TaoMaGiamGia_Load(object sender, EventArgs e)
        {
        ...
        public void Change_Language_EN()
        {
            ...
            //Label
            label_Title.Text = "Create a discount";
            label_Den.Text = "to";
            //Textbox
