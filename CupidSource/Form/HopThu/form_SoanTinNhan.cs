...
namespace Cupid
{
    public partial class form_SoanTinNhan : Form
    {
        bool kt = true;
        string tk;
        string mathu;
        ...
        public form_SoanTinNhan(string tk)
        {
            InitializeComponent();
        ...
        public form_SoanTinNhan(string tkn,int a,string tk)
        {
            InitializeComponent();
            this.tk=tk;
            this.Text = string.Empty;
            ...
            Update_GiaoDien_NgonNgu();
            label_Loi.ResetText();
            textbox_NguoiNhan.Texts = tkn;   
        }
        public form_SoanTinNhan(string mathu,string tk,string nguoinhan,string chude,string noidung)
        {
            InitializeComponent();
            this.Text = string.Empty;
            this.ControlBox = false;
            ...
            Update_GiaoDien_NgonNgu();
            label_Loi.ResetText();
            textbox_NguoiNhan.Texts = nguoinhan;
            textbox_ChuDe.Texts = chude;
            textbox_NoiDung.Texts = noidung;
            kt = false;
            this.mathu = mathu;
        }
        ...
        {
            if (textbox_ChuDe.Texts.Trim().Equals("") && textbox_NguoiNhan.Texts.Trim().Equals("") && textbox_NoiDung.Texts.Trim().Equals(""))
            {
                this.Close();
                foreach (Form f in Application.OpenForms)
                {
                    if (f.GetType() == typeof(form_HopThu))
                    {
                        ((form_HopThu)f).dataGridView.ClearSelection();
                        ((form_HopThu)f).vt = -1;
                        ((form_HopThu)f).OnOffBtn(false);
                    }
                }
            }
            else
                ...
                if (kt)
                {
                    ...
                    foreach (Form f in Application.OpenForms)
                    {
                        if (f.GetType() == typeof(form_HopThu))
                        {
                            ...
                            ((form_HopThu)f).vt = -1;
                            ((form_HopThu)f).OnOffBtn(false);
                            if (((form_HopThu)f).currentBtn == ((form_HopThu)f).button_ThuNhap)
                            {
                                ((form_HopThu)f).dataGridView.DataSource = HopThuDAO.Instance.getThuNhap(tk);
                                ((form_HopThu)f).ActivateButton(((form_HopThu)f).button_ThuNhap);
                                break;
                            }
                        }
                    }
        ...
        private void textbox_NguoiNhan_Leave(object sender, EventArgs e)
        {
            if (textbox_NguoiNhan.Texts.Trim().Length > 0)
            {
                DataTable dt = NguoiDungDAO.Instance.getNguoiDung2(textbox_NguoiNhan.Texts);

                if (dt.Rows.Count == 0)
                {
                    if (!check_ngonngu)
                    {
                        label_Loi.Text = "Người dùng không tồn tại";
                    }
                    else
                    {
                        label_Loi.Text = "User does not exist";
                    }
                }
                else
                {
        ...
        private void button_Gui_Click(object sender, EventArgs e)
        {
            if (KiemTra())
            {
                ...
                if (checkBox_Gmail.Checked == true)
                {
                    ...
                    {
                        DataProvider.Instance.ExecuteNonQuery("DELETE FROM HOP_THU WHERE MA_THU = '"+mathu+"'");
                ...
                if (kt == false)
                {
                    HopThuDAO.Instance.XoaThu(this.mathu);
                }
                
                var f1 = this.Owner;
                if (f1 != null && f1.GetType() == typeof(form_HopThu))
                {
                    ((form_HopThu)f1).OnOffBtn(false);
                }
                ...
                foreach (Form f in Application.OpenForms)
                {
                    if (f.GetType() == typeof(form_HopThu))
                    {
                        if (((form_HopThu)f).currentBtn == ((form_HopThu)f).button_DaGui)
                        {
                            ((form_HopThu)f).dataGridView.DataSource = HopThuDAO.Instance.getThuDaGui(tk);
                            ((form_HopThu)f).ActivateButton(((form_HopThu)f).button_DaGui);
                            break;
                        }
                        if (((form_HopThu)f).currentBtn == ((form_HopThu)f).button_ThuNhap)
                        {
            ...
            else
            {
                if (!check_ngonngu)
                {
                    label_Loi.Text = "Vui lòng nhập đầy đủ thông tin";
                }
                else
                {
                    label_Loi.Text = "Please enter full information";
                }
            }
        }
        ...
        private void form_SoanTinNhan_Load(object sender, EventArgs e)
        {
        ...
        {
            //Label
            label_Title.Text = "Soạn tin nhắn mới";
            //Textbox
        ...
        {
            //Label
            label_Title.Text = "Compose new message";
            //Textbox
