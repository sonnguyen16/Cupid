...
namespace Cupid
{
    public partial class form_XoaTinNhan : Form
    {
        string ma = null;
        string tk;
        bool check_giaodien, check_ngonngu;
        public form_XoaTinNhan(string ma, string tk)
        {
            InitializeComponent();
        ...
        {
            HopThuDAO.Instance.XoaThu(ma);
            var f = (form_HopThu)this.Owner;
            if (f.currentBtn == null || f.currentBtn == f.button_ChuaXem)
            {
                f.button_ChuaXem_Click(sender, e);
                f.ActivateButton(f.button_ChuaXem);
            }
            else if (f.currentBtn == f.button_DaXem)
            {
                f.button_DaXem_Click(sender, e);
                f.ActivateButton(f.button_DaXem);
            }
            else if (f.currentBtn == f.button_DaGui)
            {
                f.button_DaGui_Click(sender, e);
                f.ActivateButton(f.button_DaGui);
            }
            else
            {
                f.button_ThuNhap_Click(sender, e);
                f.ActivateButton(f.button_ThuNhap);
            }
            this.Close();
            f.OnOffBtn(false);
        }
        ...
        public void Change_Language_VI()
        {
            ...
            //Textbox
            label_NoiDung.Text = "Bạn có chắc chắn muốn xóa tin nhắn này?";
            //Button
        ...
        public void Change_Language_EN()
        {
            ...
            //Textbox
            label_NoiDung.Text = "Are you sure you want to delete this message?";
            //Button
